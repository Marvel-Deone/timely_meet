...
const BookingForm = ({ event, availability }: BookingFormProps) => {
  ...
  const onSubmitBooking = async (formData: BookingFormData) => {
    let booking_data = {};
    if (event.type === "POLL" && event.status === "finalized") {
      console.log('event.poll_options:', event.poll_options);
      booking_data = {
        eventId: event.id,
        name: formData.name,
  ...
  return (
    <Card className="border-none shadow-none bg-transparent">
      {event.type === "POLL" && event.status === "finalized" ? (
        // Skip date/time selection, go straight to details form for Poll event once finalized
        ...
        //   <div>
        //     <Input {...registerBooking("name")} placeholder="Your Name" />
        //     {bookingErrors.name && <p className="text-red-500 text-sm mt-1">{bookingErrors.name.message}</p>}
        //   </div>
        //   <div>
        //     <Input {...registerBooking("email")} type="email" placeholder="Your Email" />
        //     {bookingErrors.email && <p className="text-red-500 text-sm mt-1">{bookingErrors.email.message}</p>}
        //   </div>
        //   <div>
        //     <Textarea {...registerBooking("additional_info")} placeholder="Additional Information (optional)" />
        //   </div>
        ...
        <>
          ...
          <CardContent className="p-0">
            ...
            {(event.type === "POLL" && event.status !== "finalized" && step === 1) && (
